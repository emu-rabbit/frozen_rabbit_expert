...
- 巨匠藥 `survey-craftsmans-command-brew-guide-integrated-v1.2.0` 以滿品質為主目標，並在 bounded certificate 證明完整後綴仍可完成時，優先使用 Good／Malleable 的局部支配技能；品質提前滿會直接跳進作業 phase。exact 食藥非專家首次 frozen primary／stress 為 `768／768`、`128／128` 完成且滿品質，paired 手數 `78` 較短／`0` 較長／`690` 相同，0 safety；specialist actions 仍關閉。無 buff development 完成 `384／384`、但滿品質只有 `145／384`，所以不在穩定滿品質 envelope。這些不是實戰成功率，reserved 未執行；
