...
- `packages/solver` 現為 guide／certificate／bounded-risk 的唯一 runtime owner；五個 scenario 使用各自 versioned policy/config。網站主流程沒有「我已施放」；偏離、undo、reload 都用 actual action history 重建 memory。`3000ms` watchdog 逾時會終止 worker 並回到 `cosmic-craft-objective-lookahead-fallback-v1.5.0`；立即 worker error／null result 也會 fallback，推薦卡會顯示 elapsed、原因與 policy version。
