...
- generic evaluator 固定以 50 個 policy-effective family scenarios 去重，small 為 50×3 裝備×2 assumed worlds×1 seed＝300 episodes，full 再加 opportunity-scarce／全 Normal 並各 4 seeds＝2400。`completed` 必須分為 `requiredQuality=0` 的 progress-only delivery 與 `requiredQuality>0` 的 progress＋quality hard gate；2026-08-24 v0.5.1 paired full matrix 將前者由 `1726／1728` 提升為 `1728／1728`、0 completion regression，quality target `+1／-0`，後者仍為 `104／672`。平均 utility 差只有 `+0.000611`，95% interval 完整落在 ±`0.02` immaterial band，因此保留局部 correctness 修補但停止繼續調這個 hypothesis；不能用混合總數推論一般交貨底線、真實成功率或裝備極限。
...
- capability evidence 目前只完成 negative-only optimistic action-gain bound 與 fixed-tape clairvoyant route witness。150 個 family×equipment cells 的 horizon-80 放寬上界全部 inconclusive；Recipe 36990 的固定 tape 有可重播滿品質完成 witness，但它看得到未來且 frontier truncated。沒有 stochastic causal Bellman upper／lower bounds 前，不得宣稱接近裝備能力極限。
