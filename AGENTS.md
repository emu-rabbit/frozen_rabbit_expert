...
- native live batch ABI 已升為 transition／rollout／root v2 與 9×9 conditions；Robust fixture、54 transition cases、10 rollouts 與 12,000 root operations 已通過 TS↔Rust mechanics parity。`native-generic-episode-batch-v2` 現已承載完整 Rust closed-loop；TS policy 只作 bounded migration similarity reference，不再追逐逐招一致。使用者授權的 v0.15→v0.18 64-seed preview 已完成 384,000 paired cases／768,000 solver episodes、150／150 shards、12 分 12 秒、0 retry／timeout。v0.19 因誤把不換 condition 的 Final Appraisal 當 sampling spacer 而撤回；下一輪候選是 `generic-craft-budgeted-condition-v0.20.0`，增量 baseline 為 v0.18。v0.20 的 50-family、4-world、10-equipment、4-seed daytime gate 合計 24,000 paired cases，completion `+554／-0`、quality target `+230／-0`，足以交付 64-seed preview。Web 尚未接 Rust core；後續真正嚴格的對齊是 native Rust→同 core WASM／TS wrapper。
