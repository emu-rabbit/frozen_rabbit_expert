...
- native live batch ABI 已升為 transition／rollout／root v2 與 9×9 conditions；Robust fixture、54 transition cases、10 rollouts 與 12,000 root operations 已通過 TS↔Rust mechanics parity。`native-generic-episode-batch-v2` 現已承載完整 Rust closed-loop。`generic-craft-ts-v0.6-semantic-port-v0.21.0` 以近函式級移植追回 frozen TS 策略組合；1,000-case balanced migration gate 的 completion／stop reasons／hard-quality target 計數一致，完整 sequence `93.9%`、aligned actions `99.886%`，只宣稱 outcome parity。v0.20 作 overnight 增量 baseline；v0.21 相對 v0.20 的 Stable／Balanced／Aggressive 1,000-case gates 分別為 completion `+80／-16`、`+40／-18`、`+44／-21`。低收益的 v0.20 null-rescue bundle 已撤回。Web 尚未接 Rust core；後續真正嚴格的對齊是 native Rust→同 core WASM／TS wrapper。
