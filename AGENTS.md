...
- `packages/solver` 現為 guide／certificate／bounded-risk 的唯一 runtime owner；五個 scenario 使用各自 versioned policy/config。網站主流程沒有「我已施放」；偏離與 undo 都用 actual action history 重建 memory。進行中的 craft 不跨重新整理保存，browser storage 只保留裝備數值；`3000ms` watchdog 逾時會終止 worker 並回到 `cosmic-craft-objective-lookahead-fallback-v1.5.0`，立即 worker error／null result 也會 fallback，推薦卡會顯示 elapsed、原因與 policy version。
...
- 宇宙探索用的巨匠藥 mechanics 為作業 10000、耐久 55、品質上限 12000、`requiredQuality=0`；policy objective 才是滿品質 12000，兩者不得混為完成條件。已知收藏價值 1020–1200 對應 700–1000 分，但區間內精確換算未知；品質 10800／收藏價值 1080 只是假設線性內插的暫定 800 分 proxy，不是已驗證門檻。v1.1.0 關閉 specialist actions；食藥非專家在 assumed development primary `384／384` 與 adversarial stress `64／64` 都完成且滿品質，食藥＋專家 stats 的 aggregate 結果相同且三種 specialist actions 使用 0 次。無 buff 雖完成 `384／384`，滿品質只有 `145／384`，不得標為穩定滿品質 coverage。這些都不是實戰成功率；frozen／reserved 未執行。
