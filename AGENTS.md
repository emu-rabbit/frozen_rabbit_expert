...
- 網站已支援五個可切換 scenario：宇宙鈦鐵錠（Recipe 36282／Item 48360）、宇宙鈦鐵釘（36283／48361）、腳手架任務的硬化木板（36205／48263）與成品（36208／48311），以及 **【高難】製作工匠所需的複方藥** 的第三件「宇宙探索用的巨匠藥」（36582／48570）。目前只支援該任務第三件，不含前兩件或跨三件 mission controller。`apps/web/src/scenarios.ts` 集中綁定 recipe、`CraftObjective`、planner、物品 icon、預設裝備與 development equipment envelope。
...
- `packages/policy-lab` 保留 action-only 0／72、continuation MPC、option controller 與 specialist experiments 的正負證據，不得讓 web 反向 import training package。三 exact profiles 的 versioned development／frozen evaluator 已建立；更廣 CrafterProfile population、true condition transitions、玩家 failure／recovery traces、任意裝備 cross-profile promotion 與 OOD router仍未完成。
...
- 2026-08-12 測試價值稽核先把預設 Vitest suite 由 209 淨減為 193；2026-08-13 current checkout 完整實測為 211，其中包含 2 個跨配方 scorecard release registry guards，以及巨匠藥 exact-profile macro／observed-condition／continuous-Malleable、近滿完成效率 promotion 與球色防連點 regressions。已刪除 literal mirror、重複 wrapper／起始狀態與研究 timing oracle，將 guide／certificate／bounded-risk 測試歸回 `packages/solver`；mechanics 邊界與取整、protocol replay／undo、player traces、solver safety／certificate 等高價值 owner 保留。新增測試仍須對應可觀察 failure contract，不以數量當品質。
