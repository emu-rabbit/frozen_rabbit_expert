...
- `tools/evaluate-generic-cosmic-overnight` 提供 family × risk 的多 child 深度評測、嚴格 global budget、timeout／retry、exclusive lock、atomic shard／manifest 與同命令續跑；equipment axes 由 evaluator registry 動態取得，正式輸出保存在 Git ignored 的 `evaluation-runs/`。完整操作與 evidence 邊界只由 `.agents/workflows/run-generic-overnight-evaluation.md` 擁有。
...
- native live batch ABI 已升為 transition／rollout／root v2 與 9×9 conditions；Robust fixture、54 transition cases、10 rollouts 與 12,000 root operations 已通過 TS↔Rust parity。歷史 adaptive v1 wire 保留八 conditions，遇 Robust fail closed；native 尚未承載 generic search／Web runtime，不能因 parity 或舊效能數字提高產品成熟度。
