...
- native live batch ABI 已升為 transition／rollout／root v2 與 9×9 conditions；Robust fixture、54 transition cases、10 rollouts 與 12,000 root operations 已通過 TS↔Rust mechanics parity。`native-generic-episode-batch-v3` 現已承載完整 Rust closed-loop 與配方 random-condition mask。`generic-craft-ts-v0.6-semantic-port-v0.21.0` 以近函式級移植追回 frozen TS 策略組合；1,000-case balanced migration gate 的 completion／stop reasons／hard-quality target 計數一致，完整 sequence `93.9%`、aligned actions `99.886%`，只宣稱 outcome parity。第四批 v0.20→v0.21 的 64-seed preview 已完成 384,000 paired cases；第五批候選 `generic-craft-condition-set-portfolio-v0.22.0` 只依 hard-quality／risk／random-condition set 選擇共用路線，不讀 recipe／equipment ID。24,000-pair daytime gate 的 Stable 全部不變，Balanced／Aggressive completion 淨增 `32／37`，足以交付 v0.21→v0.22 preview。Web 尚未接 Rust core；後續真正嚴格的對齊是 native Rust→同 core WASM／TS wrapper。
