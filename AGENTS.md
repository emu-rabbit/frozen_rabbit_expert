...
- `tools/evaluate-generic-cosmic-overnight` 已有 family × risk shard、global budget、timeout／retry、exclusive lock、atomic persistence／resume 骨架，但目前 child 仍是 Node／TypeScript，不能再稱正式 Rust overnight。下一次完整夜跑暫停，直到 whole-episode Rust release binary、deterministic work contract、engine identity／parity evidence、native-only fail-closed 與 1→2→4 workers 熱校準完成；完整操作與 evidence 邊界只由 `.agents/workflows/run-generic-overnight-evaluation.md` 擁有。
...
- capability evidence 目前只完成 negative-only optimistic action-gain bound 與 fixed-tape clairvoyant route witness。舊三 profile checkpoint 的 horizon-80 結果是 150／150 inconclusive；2026-08-25 live 驗證已擴為 10 profiles × 50 families＝500 cells，projected scans `304,760,000／310,000,000`，結果仍是 0 provably impossible、0 completion impossible under relaxation、500 inconclusive。正式 10 組皆以實際 i720 Cosmic 或 i750 Stellar fixed-relic 主手工具為基礎；i780 與 CP 特化 future reference 只由 `.agents/research/open_questions.md` 擁有。這把放寬上界忽略 CP／耐久／setup 等實際代價，目前仍太鬆，不能證明可達或裝備極限。Recipe 36990 的固定 tape 有可重播滿品質完成 witness，但它看得到未來且 frontier truncated。沒有 stochastic causal Bellman upper／lower bounds 前，不得宣稱接近裝備能力極限。
...
- native live batch ABI 已升為 transition／rollout／root v2 與 9×9 conditions；Robust fixture、54 transition cases、10 rollouts 與 12,000 root operations 已通過 TS↔Rust parity。歷史 adaptive v1 wire 保留八 conditions，遇 Robust fail closed；native 尚未承載 objective／risk／safety／certificate／generic search／whole-episode 或 Web runtime。Rust generic migration 是目前最高優先 gate，完成後才在 Rust owner 繼續 hard-quality `PlannerContext` hypothesis；舊 fixed-continuation 加速與 TypeScript overnight 的耗時／溫度都不能推估 native generic 成效。
