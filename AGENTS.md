...
- `packages/policy-lab` 保留 action-only 0／72、continuation MPC、option controller 與 specialist experiments 的正負證據，不得讓 web 反向 import training package。現已加入 objective-aware finite feature v4、裝備 population／grouped-split validator、manifest-bound held-out evaluator，以及未接 runtime 的 certificate-shielded causal root MPC 研究候選；但只有結構／RNG／budget smoke，沒有 closed-loop 效果或新 promotion。三 exact profiles 仍只能標 regression-seen；真正未見裝備 population、sealed corpus contents、true condition transitions、玩家 failure／recovery traces、cross-profile promotion 與 OOD router仍未完成。
...
- 2026-08-12 測試價值稽核先把預設 Vitest suite 由 209 淨減為 193；2026-08-14 產品化 dirty checkpoint 完整實測為 43 files／264 tests，包含既有 scorecard／玩家 trace／solver safety regressions，以及 objective、裝備 population、held-out evidence、RNG consumption、native parity 與 causal planner contracts。已刪除 literal mirror、重複 wrapper／起始狀態與研究 timing oracle，將 guide／certificate／bounded-risk 測試歸回 `packages/solver`；新增測試仍須對應可觀察 failure contract，不以數量當品質。
...
- 2026-08-14 使用者把主線轉向未知裝備、更多宇宙探索高難配方與 Rust 離線加速；完整已做／未做、風險與下一步見 `solver-productization-handoff-2026-08-14.md` 的「收尾 checkpoint」。目前仍沒有新的 solver promotion。`native/craft-kernel` 只完成 shared-fixture RNG／base-gain bit parity，沒有完整 transition／search、ABI 或 runtime 加速；causal root MPC 也只屬 research candidate。接手時不得把 schema、smoke 或 optimistic beam 改寫成 runtime truth。
