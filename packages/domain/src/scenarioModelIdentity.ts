...
export const CRAFT_SCENARIO_MODEL_IDENTITY_VERSION = 'craft-scenario-model-identity-v1'
export type CraftScenarioModelContentHash = `sha256:${string}`
...
/**
 * Full content identity for a scenario's mechanics inputs and declared goal.
 * Object key order is ignored; every enumerable recipe/objective field,
 * ordered array member, source claim, and the mechanics model version is bound.
 */
export function craftScenarioModelContentHash(
  recipe: Readonly<RecipeProfile>,
  objective: Readonly<CraftObjective>,
): CraftScenarioModelContentHash {
  const canonical = canonicalJson({
    identityVersion: CRAFT_SCENARIO_MODEL_IDENTITY_VERSION,
    mechanicsModelVersion: CRAFT_MECHANICS_VERSION,
    objective,
    recipe,
  }, 'scenarioModel', new Set())
