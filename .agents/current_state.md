...
- 目前最新研究 checkpoint 是 `generic-craft-route-portfolio-v1.5.0`：回到 v1.3 的全球色與完整收尾行為，以每次推薦 scoped 的純查詢快取檢查成本；初批 1,800 cases 逐招／結果／context 精確相同，但省時仍不足，見 [v1.5 研究](../reports/generic-cosmic-overnight/v150-development/results.md)。v1.4 的起手／局部品質擴充已撤回採用，未帶入 v1.5。目前沒有交付完整 overnight。四類品質目標分開評分，不讀 recipe／equipment ID；1.x 與 Application／Cargo package／公開發布版本各自管理。
