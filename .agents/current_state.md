...
- Checkout 目前的 active candidate 是 `generic-craft-route-portfolio-v1.14.0`，v1.12 仍是採用基礎。v1.14 保留 v1.13 的通用球色工作排程：特殊球可用較便宜的代價先兌現目前或後續階段必須做的工作，完成後再恢復原 funded route；但只有目前 state 已存在 action budget 內的完整完工 witness，才啟用這套彈性排程。契約尚未付清時，直接使用 v1.12 completion-aware route；付清後，每個提案仍須讓成功／失敗分支都保有完整收尾。這是 mechanics、state、objective 與剩餘 budget 的通用邊界，不讀 recipe／equipment ID、seed、未來 RNG、評測標籤或敗場清單。
- v1.14 已完成 50 families × Balanced × E02／E03／E07／E09／E10 × `balanced-iid`／`normal-heavy-iid` × 4 seeds 的 2,000 paired／6,000 三臂 rows gate。Fresh v1.12→v1.14 為完成 1,701→1,704，paired completion `3 勝／0 敗`；滿品質 1,155→1,177，`83 勝／61 敗`；utility total `+25.386063`。560 個 hard-quality cases 的完成、滿品質與 utility 逐 case 相同，五套裝備與兩個 worlds 都是 0 completion loss；v1.14 為 0 illegal／policy-null／action-limit，推薦 p95 42.533 ms、max 138.133 ms。第三臂 Artisan Expert 預設策略移植為完成 1,715、滿品質 1,493，但有 150 個 action-limit；推薦 mean 0.130 µs、p95 0.200 µs，完成件 A p50／p95 42／70，屬外部效果與速度座標，不是 promotion floor。最終也確認 v1.12 的 2,000 個 outcome／步數／context 與既有 64-seed evidence 0 drift。完整契約見 [active brief](overnight_review_brief.md)，兩組四表見 [v1.14 三臂 gate](../reports/generic-cosmic-overnight/v114-isolated-contract-vs-v112-vs-artisan-50f-5equipment-2world-s4.md)。
...
- Solver 的數字版號只代表經驗證的有意義進步。v1.12 identity 保留為目前採用基礎；v1.14 已在 broad bounded gate 修掉已知的不合理 completion regression，並準備 fresh v1.12／v1.14／Artisan Expert 三臂 64-seed overnight。在完整結果通過前，v1.14 仍不是採用版。
...
- 目前工作入口：[overnight_review_brief.md](overnight_review_brief.md)；v1.14 完工契約是 active overnight candidate，長跑由使用者依該 brief 啟動與監控。
