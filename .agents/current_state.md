...
- 目前最新研究 checkpoint 是 `generic-craft-route-portfolio-v1.6.0`：三候選比較預算研究，1,800 cases 沒有實質成本收益且略傷品質，撤回採用，見 [v1.6 結果](../reports/generic-cosmic-overnight/v160-development/results.md)。v1.3 的完整收尾仍是後續策略基底；v1.4 局部品質擴充、v1.5 快取與 v1.6 預算縮減都未取得採用證據。目前沒有交付完整 overnight。四類品質目標分開評分，不讀 recipe／equipment ID；1.x 與 Application／Cargo package／公開發布版本各自管理。
