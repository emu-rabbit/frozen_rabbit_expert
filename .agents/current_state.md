...
- 目前開發 candidate 是 `generic-craft-route-portfolio-v1.2.0`：全部九種球色的獨立候選、完整品質收尾提案、分階段共同抽樣、無效換球／資源使用排除與局部計算重用。四類品質目標仍分開評分，候選不讀 recipe／equipment ID；1.x 與 Application／Cargo package／公開發布版本各自管理。
