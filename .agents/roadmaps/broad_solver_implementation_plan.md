...
6. **v0.21 overnight 候選已交付**：v0.20 保留作增量 baseline；`generic-craft-ts-v0.6-semantic-port-v0.21.0` 已通過 release build、Rust tests、1,000-case TS migration outcome gate 及三種 risk 的 v0.20 native A/B。Stable／Balanced／Aggressive completion 分別為 `+80／-16`、`+40／-18`、`+44／-21`，utility delta 為 `+0.0421`、`+0.0102`、`+0.0181`。另測的 v0.20 null-rescue bundle 只有 completion `+1／-0` 且多一個 failed episode，收益不足，已撤回。這足以投入 v0.20→v0.21 64-seed preview，但仍不是實戰成功率、全部 cells dominance 或接近裝備上限的證據。
