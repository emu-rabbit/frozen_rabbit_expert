...
> **歷史 snapshot。** 保存 2026-08-26 文件治理前的 broad roadmap、Rust migration checkpoints 與 v0.22 候選敘述；不能決定目前工作順序。
...
2. **完成**：objective／risk、decision memory、route options、配方 random-condition mask、RNG／transition 與 terminal 已納入 `native-generic-episode-batch-v3`；Node evaluator 不逐 action 往返，overnight parent 只做 shard／lock／timeout／retry／resume／report orchestration。
3. **完成 outcome migration baseline**：mechanics／codec／RNG／terminal 沿用 exact parity fixtures；`generic-craft-ts-v0.6-semantic-port-v0.21.0` 以近函式級 semantic port 移入 frozen TS 的 objective／safety／certificate／generic route／fallback。1,000-case balanced gate 的 completion、stop reasons 與 hard-quality target 計數一致，完整 action sequence `93.9%`、aligned actions `99.886%`；因此只宣稱 outcome parity，不把逐招 exact parity 變成永久契約。後續 Rust policy 自行演進。
...
5. **大致完成，熱 gate 待補**：`native-generic-episode-batch-v3`、paired native evaluator 與 overnight runner 已接通 release handshake、hard caps、matrix validation、binary snapshot、resume 與 1／4-worker determinism；v3 新增配方 random-condition mask，讓 recipe-conditioned selector 使用版本化 input，而不是 recipe ID。CLI 要求 `--native-preview`，避免沒有 calibration evidence 時誤稱正式夜跑。
6. **v0.22 overnight 候選已交付**：第四批 v0.20→v0.21 的 64-seed preview 已完成 384,000 paired cases／768,000 solver episodes、150／150 shards、0 retry／timeout；full report 暴露 hard-quality Balanced／Aggressive 分別淨退 `179／160`。`generic-craft-condition-set-portfolio-v0.22.0` 以共用 solver 加上 hard-quality／risk／random-condition-set selector，只在三組重複出現的 Centered＋Pliant 球色家族切回 v0.20 路線，不讀 recipe／equipment ID。24,000-pair full daytime gate 的 Stable `+0／-0`；Balanced `+60／-28`、淨 `+32`；Aggressive `+67／-30`、淨 `+37`，progress-only 全部不變。由已保存第四批 paired rows 重算第五批 selector，64-seed 預期 Balanced 淨 `+533`、Aggressive 淨 `+548`，足以投入 v0.21→v0.22 preview；仍不是實戰成功率、逐 cell dominance 或接近裝備上限的證據。
