...
巨匠藥 `survey-craftsmans-command-brew-guide-integrated-v1.1.0` 保留單一 recipe policy identity，不建立「專家／非專家兩套策略器」。config 以 all-Normal 可證 quality-first route 作穩定主線；一般 route 若準備以 Malleable 作業提前完成且品質仍低於目標，只有 bounded certificate 能證明先到 12000（其次是暫定 10800 guardrail）後仍可完工時才延後收尾，否則走安全 contingency。development 比較顯示食藥＋專家 stats 與食藥非專家結果完全相同，且 `carefulObservation`／`heartAndSoul`／`quickInnovation` 都使用 0 次；v1.1.0 因此明確關閉 `allowSpecialistActions` 與 `useSpecialistFinisher`，checkpoint `827cf73` 的 v1.0.0 只保留歷史基準。

在三個 assumed Normal／Good／Malleable primary profiles 各 128 seeds（合計 384）與兩個 adversarial stress profiles 各 32 seeds（合計 64）中，食藥非專家完成且滿品質分別為 `384／384`、`64／64`；食藥＋專家 stats 的 aggregate metrics 完全相同。無 buff primary 雖完成 `384／384`，滿品質只有 `145／384`，所以不列入巨匠藥穩定滿品質 development envelope。這些 profiles 已參與開發、不是玩家自然轉移或實戰成功率；frozen-validation 與 reserved-final 都未執行，不構成正式 promotion 或 universal coverage。
