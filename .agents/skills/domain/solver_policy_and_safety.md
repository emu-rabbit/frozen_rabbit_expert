...
- 宇宙探索用的巨匠藥：目前只支援 **【高難】製作工匠所需的複方藥** 第三件，不含前兩件或跨件任務規劃。Recipe 36582 的最低品質為 0、品質上限為 12000；作業達 10000 即 mechanics completion，policy 另以滿品質 12000 為主要目標。已知收藏價值 1020–1200 對應 700–1000 分，但區間內精確換算未知；品質 10800／收藏價值 1080 只是假設線性內插的暫定 800 分 proxy，不能寫成已驗證門檻。若繼續加工已失去可證完工路線，completion safety 優先於 proxy／滿品質。
...
巨匠藥 `survey-craftsmans-command-brew-guide-integrated-v1.2.0` 保留單一 recipe policy identity，不建立「專家／非專家兩套策略器」。config 以 all-Normal 可證 quality-first route 作穩定主線；v1.2.0 在每一步重證完整剩餘路線後，允許 Good `Precise Touch` 取代「品質更低且 CP／耐久不更省」的固定品質技能，也允許 Good `Intensive Synthesis`／Malleable 作業技能在局部作業量更高、CP／耐久不增加且固定後綴仍可完成時取代原技能。若這些替換令品質提前滿 12000，route memory 直接跳到已證明的作業 phase，不會掉入 generic fallback。一般 route 若準備以 Malleable 作業提前完成且品質仍低於目標，仍只有 bounded certificate 能證明先到 12000（其次是暫定 10800 guardrail）後可完工時才延後收尾。specialist actions 仍明確關閉；v1.1.0 與 v1.0.0 保留為 release history。

在 exact 食藥非專家 development primary／stress 中，candidate 與固定 v1.1 路線皆為 `384／384`、`64／64` 完成且滿品質；primary paired 手數為 `56` 較短／`0` 較長／`328` 相同，平均 `25.336→25.167`，明確 condition uses `447→853`，paired `215` 更多／`0` 更少。首次使用且不再回看的 frozen primary／stress 也維持 `768／768`、`128／128` 完成且滿品質；primary paired 手數 `78／0／690`，平均 `25.255→25.139`，明確 condition uses `928→1717`，paired `416` 更多／`0` 更少。0 safety violation。這支持 `near-perfect-condition-responsive-efficiency` promotion basis；profiles 仍是 assumed IID sensitivity、不是自然轉移或實戰成功率。reserved-final 未執行，無 buff 仍不在穩定滿品質 envelope。
