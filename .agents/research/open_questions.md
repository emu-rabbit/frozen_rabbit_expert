...
| RQ-11 | 將 Raphael 與較深 Rust teacher 蒸餾成小型候選排序器，能否改善球色判斷而不傷害食藥基本功、hard-quality 與 Stable？ | 決定學習式 scorer 能否從 bounded implementation 進入大量訓練 | 路線感知標籤、teacher-vs-v1.12 closed loop、連續 mechanics 裝備抽樣、leave-one-anchor-out、成組保留集、artifact／latency | [學習式候選排序器計畫](learned_candidate_scorer_plan.md)＋Rust evaluation report |
