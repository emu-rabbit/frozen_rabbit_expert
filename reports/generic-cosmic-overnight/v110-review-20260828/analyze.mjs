...
const runId = 'generic-native-v110-perf-vs-v030-64seed-20260827'
...
const timingChunks = new Map()
const slowRows = []
const workerAttempts = {}
const attemptOutcomes = {}
for (const sh of manifest.shards) for (const a of sh.attempts) {
  const key = String(a.configuredWorkerCount ?? 'unknown')
  workerAttempts[key] ??= {completed:0,other:0,totalMs:0}
  workerAttempts[key][a.outcome==='completed'?'completed':'other']++
  workerAttempts[key].totalMs += a.durationMs ?? 0
  attemptOutcomes[a.outcome] = (attemptOutcomes[a.outcome]??0)+1
}
function timingAdd(key, rows) {
  if(!timingChunks.has(key)) timingChunks.set(key,[])
  timingChunks.get(key).push(Float64Array.from(rows.flatMap(r=>r.recommendationDurationsNs)))
}
...
function addArm(g,r) {
  ...
  const hq=estimateHqChancePercent(r.quality,r.qualityMaximum)
...
for (const entry of manifest.shards) {
  ...
  const worker=shard.timingContext?.configuredWorkerCount ?? 'unknown'
  for(const arm of ['baseline','candidate']) {
    const armRows=shard.report.rows.filter(r=>r.arm===arm)
    timingAdd(arm,armRows)
    timingAdd(arm+'/workers/'+worker,armRows)
    for(const row of armRows) if(row.recommendationMaxNs>=1e9) slowRows.push({...identity(row),arm,maxMs:row.recommendationMaxNs/1e6,callsOver1s:row.recommendationDurationsNs.filter(n=>n>=1e9).length,callsOver3s:row.recommendationDurationsNs.filter(n=>n>=3e9).length})
  }
  ...
  for(const {baseline:b,candidate:c} of paired.values()){
    ...
    const keys=['all',`contract/${contract}`,`kind/${kind}`,`risk/${c.risk}`,`world/${c.worldId}`,`equipment/${e}`,`family/${f}`,
      ...
      `family-equipment/${f}/${e}`,`contract-risk-world/${contract}/${c.risk}/${c.worldId}`,`contract-equipment-world/${contract}/${e}/${c.worldId}`]
    if(c.risk==='balanced'&&c.worldId==='balanced-iid'&&(e==='E02'||e==='E09')) {
      keys.push(`focus/${contract}/${e}`,`focus-family/${f}/${e}`,`focus/${contract}/both`,`focus-kind/${kind}/both`)
      if(!focusBlocks.has(f)) focusBlocks.set(f,{contract,values:Array.from({length:64},()=>[0,0,0])})
      const v=focusBlocks.get(f).values[c.seedIndex]
      v[0]+=(Number(c.terminal==='completed')-Number(b.terminal==='completed'))/2
      v[1]+=(c.completedObjectiveUtility-b.completedObjectiveUtility)/2
      v[2]++
    }
    ...
    const d=c.completedObjectiveUtility-b.completedObjectiveUtility
    // Keep diagnostic evidence bounded: every delivery loss/action limit, plus focused quality losses.
    if(completionLoss||c.stopReason==='action-limit'||(d< -1e-12&&c.risk==='balanced'&&c.worldId==='balanced-iid'&&(e==='E02'||e==='E09')))exceptions.push({...identity(c),completionLoss,fullLoss,newBad,delta:d,baseline:slim(b),candidate:slim(c)})
    if(d>1e-12){
...
const overviewDir=path.join(root,'.tmp/v110-review-overview')
...
// Two-stage paired family/seed bootstrap; each selected seed block keeps E02 and E09 together.
...
const bootstrap={}
for(const contract of ['hard-quality','progress-only']){
  const blocks=[...focusBlocks.values()].filter(x=>x.contract===contract).map(x=>x.values)
  assert(blocks.every(b=>b.every(v=>v[2]===2)))
  const draws=[[],[]]
  for(let rep=0;rep<2000;rep++){
    ...
    for(let f=0;f<blocks.length;f++){
      ...
      for(let seed=0;seed<64;seed++){
        const v=block[Math.floor(rng()*64)]
        sums[0]+=v[0];sums[1]+=v[1]
      }
    }
    sums.forEach((x,i)=>draws[i].push(x/(blocks.length*64)))
  }
  bootstrap[contract]={families:blocks.length,seedBlocksPerFamily:64,replicates:2000,bootstrapSeed:1101,rng:'mulberry32 (shared stream, hard-quality then progress-only)',
    completionDelta95:draws[0].sort((a,b)=>a-b).filter((_,i)=>i===50||i===1950),
    utilityDelta95:draws[1].sort((a,b)=>a-b).filter((_,i)=>i===50||i===1950)}
}
const timings={}
for(const [key,chunks] of timingChunks){
  const n=chunks.reduce((s,a)=>s+a.length,0)
  const all=new Float64Array(n);let offset=0
  for(const a of chunks){all.set(a,offset);offset+=a.length}
  all.sort()
  timings[key]={n,percentileMethod:'nearest-rank',p50Ms:all[Math.ceil(n*.50)-1]/1e6,p95Ms:all[Math.ceil(n*.95)-1]/1e6,p99Ms:all[Math.ceil(n*.99)-1]/1e6,maxMs:all[n-1]/1e6,
    over1s:all.reduce((s,x)=>s+Number(x>=1e9),0),over3s:all.reduce((s,x)=>s+Number(x>=3e9),0)}
}
const attempts=manifest.shards.flatMap(s=>s.attempts)
const intervals=attempts.filter(a=>a.finishedAt).map(a=>[Date.parse(a.startedAt),Date.parse(a.finishedAt)]).sort((a,b)=>a[0]-b[0])
let activeWallMs=0, start=null,end=null
for(const [a,b] of intervals){if(start===null){start=a;end=b}else if(a<=end){end=Math.max(end,b)}else{activeWallMs+=end-start;start=a;end=b}}
if(start!==null)activeWallMs+=end-start
const result={bootstrap,timings,slowRows:slowRows.sort((a,b)=>b.maxMs-a.maxMs),workerAttempts,attemptOutcomes,activeAttemptUnionWallMs:activeWallMs,runId,configFingerprint:config.configFingerprint,binarySha256:execution.binarySha256,evaluatorBundleSha256:sha256File(bundle),
...
const lines=['# v1.1 逐家族索引','', '> 由 analyze.mjs 重算；每列包含 10 equipment × 3 risk × 4 assumed worlds × 64 seeds，共 7,680 pairs。這張表僅用於定位家族，不能代替格內與逐案分析。pp 是百分點。','',
...
console.log(JSON.stringify({validation:result.validation,bootstrap,timings,workerAttempts,attemptOutcomes,activeAttemptUnionWallMs:activeWallMs,exceptionCases:exceptions.length},null,2))
