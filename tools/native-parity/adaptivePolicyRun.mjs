...
const buildDirectory = mkdtempSync(path.join(temporaryRoot, 'native-adaptive-policy-parity-'))
const output = path.join(buildDirectory, 'verify.mjs')

try {
  const build = spawnSync(process.execPath, [
    path.join(root, 'node_modules', 'rolldown', 'bin', 'cli.mjs'),
    path.join(root, 'tools', 'native-parity', 'adaptivePolicyVerify.ts'),
    '--file', output,
    '--format', 'esm',
    '--platform', 'node',
  ...
  if (build.status !== 0) throw new Error(`adaptive-policy parity bundle failed with status ${build.status}`)
  ...
  if (run.status !== 0) throw new Error(`adaptive-policy parity verification failed with status ${run.status}`)
} finally {
  ...
  ) throw new Error(`refusing to remove parity directory outside .tmp: ${buildDirectory}`)
